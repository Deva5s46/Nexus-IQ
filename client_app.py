...
from models import db, User, Message, Channel, Notification, ChannelVisit, DMPermission, Workspace, WorkspaceAccess, Team, GroupMember, ChannelRolePermission, migrate_encrypted_fields, ensure_performance_indexes
from db_config import configure_sqlalchemy, enable_sqlcipher_pragmas
from public_urls import get_admin_public_base_url
import datetime
...
socketio = SocketIO(client_app, cors_allowed_origins="*")
...
with client_app.app_context():
    enable_sqlcipher_pragmas(client_app, db)
    # NEW MIGRATION: Add is_restricted column if it doesn't exist
    try:
    ...
    # Migration: Set default names for users who don't have one
    users_without_names = User.query.filter((User.name == None) | (User.name == "")).all()
    for u in users_without_names:
        u.name = u.email.split('@')[0]

    Channel.query.filter((Channel.post_permission_mode == None) | (Channel.post_permission_mode == "")).update(
        {Channel.post_permission_mode: 'all_visible'},
        synchronize_session=False
    )
    
    # NEW MIGRATION: Ensure all users have access to their assigned workspace
    ...
    # Ensure Entegrasources is public by default
...
def can_user_dm_target(sender, target):
    ...
    target_is_lead = (target.team_role or '').strip().lower() == 'teamlead' or target.role in ('admin', 'superadmin')
    has_grant = DMPermission.query.filter_by(user_id=sender.id, target_id=target.id).first() is not None
...
def get_channel_base_query(user):
    ...
    if user.workspace_id == 1 and user.team_name:
        explicit_group_ids = [row.group_id for row in db.session.query(GroupMember.group_id).filter(GroupMember.user_id == user.id).all()]
        if explicit_group_ids:
            query = query.filter(or_(
                Channel.team_name == user.team_name,
                Channel.id.in_(explicit_group_ids)
            ))
        else:
...
def apply_channel_visibility_filter(query, user):
    ...
    custom_visible_ids = db.session.query(GroupMember.group_id).filter(GroupMember.user_id == user.id)
...
def can_user_view_channel(user, channel):
    ...
    if (channel.visibility or '').strip().lower() == 'custom':
        if user.role in ['admin', 'superadmin']:
            return True
        return user.id in get_channel_explicit_member_ids(channel)
    return apply_channel_visibility_filter(
        Channel.query.filter(Channel.id == channel.id),
        user
    ).first() is not None
...
# --- REAL-TIME SIGNALING (For Voice/Video Calls) ---
@socketio.on('join')
def on_join(data):
    if not _require_socket_auth():
        return
    room = f"user_{current_user.id}"
    join_room(room)
    client_app.logger.debug("User joined personal signalling room: %s", room)
...
def handle_call(data):
    ...
    if not target or not can_user_dm_target(current_user, target):
        return
...
def handle_answer(data):
    ...
    room = f"user_{target_id}"
    emit('call-answered', {'from': current_user.id, 'answer': answer}, room=room)
...
def handle_ice(data):
    ...
    room = f"user_{target_id}"
    emit('ice-candidate', {'from': current_user.id, 'candidate': candidate}, room=room)

# --- HUDDLE / GROUP CALL SIGNALING ---
@socketio.on('join-huddle')
def on_join_huddle(data):
    if not _require_socket_auth():
        return
    room_name = data.get('room_name')
    channel = get_channel_in_context(current_user, channel_name=room_name) if room_name else None
    if channel and can_user_view_channel(current_user, channel):
        room = f"huddle_{room_name}"
        join_room(room)
        client_app.logger.debug("User %s joined huddle room %s", current_user.email, room)
        emit('huddle-status-update', {
...
def on_huddle_action(data):
    ...
    room = f"huddle_{room_name}"
    client_app.logger.debug("Huddle action [%s] from %s in room %s", action, current_user.email, room)
    emit('huddle-status-update', {
        'user': current_user.email,
        'user_id': current_user.id,
        'action': action,
...
def on_huddle_signal(data):
    if not _require_socket_auth():
        return
    # Multi-user mesh signaling: sender -> target_id (in a huddle context)
    target_id = _resolve_socket_target_id(data)
    if not target_id:
        return
    target = db.session.get(User, target_id)
    ...
    sender_id = current_user.id
    room = f"user_{target_id}"
    client_app.logger.debug("Huddle signal from %s -> Room %s", current_user.email, room)
    emit('huddle-signal', {
        'from': sender_id,
        'from_email': current_user.email,
...
def handle_end(data):
    ...
    if not target or not can_user_dm_target(current_user, target):
        return
    room = f"user_{target_id}"
    emit('call-ended', {'from': current_user.id}, room=room)
